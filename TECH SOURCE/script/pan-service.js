...
function openDownloadFromQuery() {
  const params = new URLSearchParams(window.location.search);
  const ack = params.get("download");

  if (!ack) return;

  openDownloadPopup();
  document.getElementById("ackNo").value = ack;
  document.getElementById("downloadMsg").innerText = "ACK loaded. Click Submit to download PAN.";
}
