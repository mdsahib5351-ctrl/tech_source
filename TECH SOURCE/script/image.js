...
const BG_KEY = "imageToolsBgApiKey";
const BG_PROVIDER_KEY = "imageToolsBgProvider";
...
function initToolSettings() {
  if (bgProvider) {
    bgProvider.value = localStorage.getItem(BG_PROVIDER_KEY) || "removebg";
    bgProvider.addEventListener("change", () => {
      localStorage.setItem(BG_PROVIDER_KEY, bgProvider.value);
    });
  }

  if (bgApiKey) {
    bgApiKey.value = localStorage.getItem(BG_KEY) || "";
    bgApiKey.addEventListener("input", () => {
      localStorage.setItem(BG_KEY, bgApiKey.value.trim());
    });
  }
}

function setToolLoading(button, isLoading, loadingText) {
  if (!button) return;
  if (isLoading) {
...
async function removeBackground() {
  ...
  const provider = bgProvider?.value || "removebg";
  ...
  setToolLoading(bgRemoveBtn, true, "Removing...");
  bgResult.innerHTML = `<div class="tool-note">Processing image...</div>`;

  try {
    const output = provider === "clipdrop"
      ? await removeBgWithClipdrop(file, apiKey)
      : await removeBgWithRemoveBg(file, apiKey);

    const url = URL.createObjectURL(output);
    const name = file.name.replace(/\.[^.]+$/, "-no-bg.png");

    bgResult.innerHTML = "";
    bgResult.appendChild(createDownloadCard(url, name, "Background removed"));
    showToast("Background removed");
  } catch (error) {
    bgResult.innerHTML = `<div class="tool-note error">${error.message || "Background remove failed"}</div>`;
    showToast(error.message || "Background remove failed", "error");
  } finally {
    setToolLoading(bgRemoveBtn, false);
  }
}

async function removeBgWithRemoveBg(file, apiKey) {
  const form = new FormData();
  ...
  if (!response.ok) {
    throw new Error(await readApiError(response, "remove.bg failed"));
  }

  return response.blob();
}

async function removeBgWithClipdrop(file, apiKey) {
  const form = new FormData();
  form.append("image_file", file);
  ...
  if (!response.ok) {
    throw new Error(await readApiError(response, "Clipdrop failed"));
  }
...
async function readApiError(response, fallback) {
  ...
  try {
    ...
    return data.errors?.[0]?.title || data.error || data.message || `${fallback} (${response.status})`;
  } catch {
    return text.slice(0, 120);
  }
}

async function imagesToPdfFromFiles() {
  const files = [...(pdfImageFiles?.files || [])];
  if (!files.length) {
    showToast("Select image files first", "error");
    return;
  }

  setToolLoading(makePdfBtn, true, "Creating...");
  imagePdfResult.innerHTML = `<div class="tool-note">Building PDF...</div>`;
  ...
  try {
    ...
    for (const [index, file] of files.entries()) {
      ...
      fitImageOnPdf(pdf, image, imageData);
    }

    pdf.save("images.pdf");
    imagePdfResult.innerHTML = `<div class="tool-note">PDF downloaded with ${files.length} image(s).</div>`;
    showToast("PDF downloaded");
  } catch {
    imagePdfResult.innerHTML = `<div class="tool-note error">Image to PDF failed.</div>`;
    showToast("Image to PDF failed", "error");
  } finally {
    setToolLoading(makePdfBtn, false);
  }
}

function fitImageOnPdf(pdf, image, imageData) {
  const pageWidth = pdf.internal.pageSize.getWidth();
  const pageHeight = pdf.internal.pageSize.getHeight();
  const margin = 10;
  const maxWidth = pageWidth - margin * 2;
...
async function pdfToJpeg() {
  ...
  setToolLoading(pdfToJpegBtn, true, "Converting...");
  pdfJpegResult.innerHTML = `<div class="tool-note">Rendering PDF pages...</div>`;

  try {
    const buffer = await file.arrayBuffer();
    const pdf = await pdfjsLib.getDocument({ data: buffer }).promise;
    pdfJpegResult.innerHTML = "";
    ...
    for (let pageNumber = 1; pageNumber <= pdf.numPages; pageNumber += 1) {
      ...
      const name = `${file.name.replace(/\.pdf$/i, "")}-page-${pageNumber}.jpg`;
      pdfJpegResult.appendChild(createDownloadCard(url, name, `Page ${pageNumber}`));
    }

    showToast("PDF converted");
  } catch {
    pdfJpegResult.innerHTML = `<div class="tool-note error">PDF to JPEG failed.</div>`;
    showToast("PDF to JPEG failed", "error");
  } finally {
    setToolLoading(pdfToJpegBtn, false);
  }
}

function createDownloadCard(url, fileName, label) {
  const card = document.createElement("div");
...
makePdfBtn?.addEventListener("click", imagesToPdfFromFiles);
pdfToJpegBtn?.addEventListener("click", pdfToJpeg);
...
function openToolFromHash() {
  ...
  if (hash === "home" || hash === "upload" || hash === "history") {
    activateTab(hash);
    return;
  }

  const tool = document.getElementById(hash);
  if (!tool) return;

  activateTab("tools");
  setTimeout(() => tool.scrollIntoView({ behavior: "smooth", block: "start" }), 100);
}
...
document.querySelectorAll("[data-open-tool]").forEach((card) => {
  card.addEventListener("click", () => {
    const target = card.dataset.openTool;
