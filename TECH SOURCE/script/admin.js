...
function isPanApproved(data) {
  return safe(data.status || "").toLowerCase().includes("approved") || Boolean(data.documentUrl);
}
...
_यह एक अनुस्मारक है कि ऊपर उल्लिखित बिल की बकाया राशि (${dueAmount}rs) अभी तक भुगतान नहीं हुई है। कृपया सुनिश्चित करें कि भुगतान निर्धारित तिथि(${formattedDate}) से पहले कर दिया जाए, ताकि सेवा में कोई रुकावट न आए।_

_Dear_ *${customerName}*,
_This is a reminder that the above-mentioned bill has a pending amount(₹${dueAmount}). Please ensure that the payment is made before the due date(${formattedDate}) to avoid any service disruption_

_आप भुगतान इस लिंक पर क्लिक करके आसानी से कर सकते हैं / You can make the payment by clicking here:_

*🢃Pay Now🢃*
https://mdsahib5351-ctrl.github.io/TECH-SOURCE-/billview.html?id=${docId}

~यदि आपने पहले ही भुगतान कर दिया है, तो कृपया इसे अनदेखा करें।If you have already made the payment, please ignore this message.~
*धन्यवाद* / *Thank you*`
  );
...
function renderPanApplications(applications) {
  ...
  applications.forEach((d) => {
    const tr = document.createElement("tr");
    const isApproved = isPanApproved(d);
    const panActionButton = isApproved
      ? `<button class="textBtn panRemindBtn" type="button" title="Send WhatsApp reminder" onclick="sendPanReadyReminder('${d.id}')">Remind</button>`
      : `<button class="editBtn" type="button" title="Edit PAN application" onclick="editPan('${d.id}')">Edit</button>`;
...
function getPanApplicationType(data) {
  if (data.applicationType) return data.applicationType;
  return data.isMinor ? "Minor PAN" : "New PAN";
}
...
function buildPanReadyMessage(data) {
  const name = safe(data.name) === "-" ? "Customer" : safe(data.name);
  const ackNo = safe(data.ackNo) === "-" ? "N/A" : safe(data.ackNo);
  const applicationType = getPanApplicationType(data);
  const downloadUrl = new URL(`pan-service.html?download=${encodeURIComponent(ackNo)}`, window.location.href).href;
...
Dear ${name},
Your PAN card is ready. Click the link below to download your PAN card.

Click Now:
${downloadUrl}

Thanks for using Tech Source.`;
}

function sendPanReadyReminder(docId) {
  const data = latestPanApplications.find((item) => item.id === docId);
  ...
  if (!isPanApproved(data)) {
    alert("Reminder sirf approved PAN application ke liye available hai");
    return;
  }

  if (!data.documentUrl) {
    alert("PAN download URL available nahi hai. Pehle PAN document upload karein.");
    return;
  }

  const mobile = cleanWhatsAppNumber(data.phone || data.mobile);
  const text = encodeURIComponent(buildPanReadyMessage(data));
  const url = mobile
